...
const columnMapping = {
  ...
  education: "education",
  "# eligible": "eligible",
  "Date Validated at School": "validatedAtSchool",
  "# Date Validated at School": "noValidatedAtSchool",
  "# CTEF received at SA": "ctefReceivedBySA",
  "# CTEF Serial number": "ctefSerialNumber",
  "Date corrected on SSSAMS": "dateCorrectedSSSAMS",
  "# Date corrected on SSSAMS": "noDateCorrectedSSSAMS",
  "Date Approved": "dateApproved",
  "# Date Approved": "noDateApproved",
  "# Signature on Payment List": "signatureOnPaymentList",
  "Date Collected at School": "dateCollectedAtSchool",
  "# Date Collected at School": "noDateCollectedAtSchool",
...
export async function GET(request) {
  ...
  const filePath = `./ct-report-schools-${year}.xlsx`;
  ...
  if (fs.existsSync(filePath)) {
    ...
    for (let rowIndex = range.s.r + 1; rowIndex <= maxRow; rowIndex++) {
      ...
      try {
        const tracker = await schoolTrackerRepository.save(rowObject)
        tracker.id = tracker[EntityId]
        await schoolTrackerRepository.save(tracker)
      } catch (error) {
        console.log(error);
      }
    }
...
export async function DELETE(request) {
  ...
  const year = searchParams.get("year");
  if (year >= 2021) {
    const ids = await schoolTrackerRepository.search().where('year').eq(year).return.allIds()

    await schoolTrackerRepository.remove(...ids)
